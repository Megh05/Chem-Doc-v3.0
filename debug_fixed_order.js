const content = `### 1.1 Product identifier

Product name
CAS No.

Sodium Hyaluronate
$9067-32-7$
1.2 Details of the supplier of the safety data sheet

Manufacturer:
Address:

Telephone:
Fax:
E-Mail:
Shandong Focusfreda Biotech Co., Ltd
Add:New Economic Development Zone of High
Speed Rail,Qufu,Jining,Shandong,China
4000905566
+865373195599
sales@focusfreda.com
1.3 Emergency telephone number

Emergency response service: 4000905566`;

console.log('Original content:');
console.log(content);
console.log('Length:', content.length);
console.log('\n---\n');

let cleaned = content;

// Check if it's Section 1 content
const isSection1Content = cleaned.includes('Product identifier') || 
                         cleaned.includes('Product name') || 
                         cleaned.includes('CAS No') ||
                         cleaned.includes('Manufacturer') ||
                         cleaned.includes('Emergency telephone');

console.log('Is Section 1 content?', isSection1Content);

if (isSection1Content) {
  console.log('✅ Using Section 1 cleaning (headers first)');
  
  // Step 1: Remove headers FIRST before doing aggressive cleaning
  console.log('Step 1: Remove headers first');
  cleaned = cleaned
    .replace(/^#+\s*Section\s+\d+[^#]*$/gmi, '') // Remove section headers like "## Section 1 - Title"
    .replace(/^#+\s*\d+\.\s*[^#\n]*$/gm, '') // Remove numbered headers like "### 1.1 Title" (single line only)
    .replace(/^#{1,6}\s*/gm, '') // Remove markdown header symbols
    .trim();
  
  console.log('After removing headers:');
  console.log(cleaned);
  console.log('Length:', cleaned.length);
  console.log('\n---\n');
  
  // Step 2: Then do minimal LaTeX cleaning
  console.log('Step 2: LaTeX cleaning');
  cleaned = cleaned
    .replace(/\\beta/g, 'β')
    .replace(/\\rightarrow/g, '→')
    .replace(/\\geqslant/g, '≥')
    .replace(/\\leqslant/g, '≤')
    .replace(/\\quad/g, ' ')
    .replace(/\\left\(/g, '(')
    .replace(/\\right\)/g, ')')
    .replace(/\\mathrm\{([^}]+)\}/g, '$1')
    .replace(/\\[a-zA-Z]+\{[^}]*\}/g, '') // Remove other LaTeX commands
    .replace(/\$([^$]+)\$/g, '$1') // Remove LaTeX math formatting like $9067-32-7$
    .replace(/\\[a-zA-Z]+/g, '') // Remove remaining LaTeX commands
    .replace(/\{([^}]+)\}/g, '$1') // Remove remaining braces
    .replace(/\s+/g, ' ') // Multiple spaces to single space
    .trim();
  
  console.log('After LaTeX cleaning:');
  console.log(cleaned);
  console.log('Length:', cleaned.length);
  console.log('\n---\n');
  
  console.log('Final result length > 20?', cleaned.length > 20);
} else {
  console.log('❌ Not Section 1 content, using aggressive cleaning');
}

