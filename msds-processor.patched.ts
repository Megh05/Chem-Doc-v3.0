...
function sliceSectionsVerbatim(fullText: string, headings: HeadingHitVerbatim[]): SectionBlockVerbatim[] {
  ...
  return [...map.entries()].sort((a,b)=>a[0]-b[0]).map(([number, content]) => ({ number, content }));
}

/** Minimal pre-clean for verbatim mode: only normalize EOLs (no content changes) */
function preCleanVerbatim(text: string): string {
  return (text ?? '').replace(/\\r\\n?/g, '\\n');
}
